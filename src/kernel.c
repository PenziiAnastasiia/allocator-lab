...
_Noreturn void failed_kernel_alloc(void) {
...
_Noreturn void failed_kernel_free(void) {
...
_Noreturn void failed_kernel_reset(void) {
...
#endif
  void *ptr = mmap(NULL, size, PROT_READ|PROT_WRITE, MMAP_FLAG_ANON|MAP_PRIVATE,
      -1, 0);
  if (ptr == MAP_FAILED) {
    if (errno == ENOMEM) {
      return NULL;
